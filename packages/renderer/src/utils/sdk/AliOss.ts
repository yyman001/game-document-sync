...
export class AliOssSDK {
  ...
  constructor ({ accessKeyId, accessKeySecret, bucket, rootDirectoryName = '' }: any) {
   ...
   * @returns
   */
  tarnslateData (list: { name: any; url: any; lastModified: any; size: any; etag: any }[]) {
  ...
  getDirectoryStructure = async () => {
    ...
    try {
      ...
      console.error('getDirectoryStructure:', e)
    }
    this.loading = false
    return {
  ...
  /**
    ...
    * @returns {Promise<Boolean>} - 是否成功
    */
  downloadFile = async (coludFilename: string, writeFilePath: string, cb: Function): Promise<boolean> => {
    try {
